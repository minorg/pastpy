...
class _Template(ABC):
    ...
    def _new_metadata(self, *, out_dir_relpath, active_nav_item="home"):
        ...
        builder.root_relative_href = os.path.relpath(
            self.__configuration.output_dir_path, os.path.join(self.__configuration.output_dir_path, out_dir_relpath)).replace(os.path.sep, '/')
    ...
    def _render_mustache(self, *, context, out_file_relpath, template_name):
        renderer = \
            pystache.Renderer(
                missing_tags='strict',
                search_dirs=(self.__configuration.template_dir_path,)
            )
        ...
        if not os.path.isdir(out_dir_path):
            ...
            self.__logger.debug("created directory %s", out_dir_path)
